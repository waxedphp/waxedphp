...
class Base {
  ...
  */
  public function setup(array $cfg): object {
    ...
    } else if ((isset($cfg['development'])) && (is_string($cfg['development']))) {
      $this->setDevelopment($cfg['development']);
    } else if ((isset($cfg['development'])) && (intval($cfg['development'])==1)) {
    ...
    if ((isset($cfg['vocab_path'])) && (is_string($cfg['vocab_path']))) {
      $this->vocab->setPath($cfg['vocab_path']);
    };
    if ((isset($cfg['nodejs_path'])) && (is_string($cfg['nodejs_path']))) {
      $this->setNodeJsPath($cfg['nodejs_path']);
    };
    if ((isset($cfg['vendor_path'])) && (is_string($cfg['vendor_path']))) {
      $this->setVendorPath($cfg['vendor_path']);
    };
  ...
  * @return object
  */
  public function configure(?array $cfg = null, string $name = 'waxed'): object {
    //if (!isset($cfg[$name])) return $this;
    if ((is_array($cfg))&&(is_array($cfg[$name]))) {
      $this->setup($cfg[$name]);
      return $this;
    }
    $config = new Config();
    //print_r($config->getConfig());die();
  ...
  */
  ...
  /**
  * method setVendorPath
  *
  * @param string $s
  * @return object
  */
  private function setVendorPath(string $s): object {
    $this->plugin->setVendorPath($s);
    return $this;
  }
  ...
  */
  private function setDevelopment(bool|string $b): object {
    ...
    if (self::$development) {
      ...
      //echo __FUNCTION__;print_r( $this->plugin );//die();
    }
    return $this;
  }
  ...
  */
  function setter(string $name): ?object {
      switch($name){
        ...
        case 'Article':
          return new \Waxedphp\Waxedphp\Php\Setters\Article($this);
      }
  ...
  */
  public function __call(string $name, array $arguments): mixed {
    ...
    $availableReturning = ['view', 'response', 'responseArray','renderBlock'];
