...
class Reaction {
  ...
  * get base
  *
  * @return \JasterStary\Waxed\Php\Base
  */
  ...
  */
  ...
  private function getBlocks(): array {
    $a = [];$re = $this->reactions;
    foreach ($re as $k => $v) {
      if ((is_object($v))&&(is_callable([$v, 'toBlock']))) {
        $v->toBlock($a);
      }
    }
    return $a;
  ...
  * @return object
  */
  function display(array $RECORD, ?string $template = null, int|bool $onTime = false, ?string $whereId = null, bool $append = false): object  {
  ...
  */
  public function view(string $id, string|array $template, array $data = [], $js = null): string {
    ...
    if (is_array($template)){
      ...
      if (isset($template['htmlpath'])) $t = $this->base->design->mode('html')->route(explode('/',$template['htmlpath']))->GET();
