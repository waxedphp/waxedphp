...
class Action {
  ...
  */
  function dispatch(array $data) {
    ...
    $getData = null;
    $a = parse_url($data['action']);
    if (!empty($a['query'])) {
      parse_str($a['query'], $getData);
    }
    $action = explode('/', $a['path']);
    ...
    */
    ...
    if (isset($this->methods[$actb])) {
      $o = call_user_func($this->methods[$actb], $data, $getData);
      return $o;
    };
    // do we have present some default method for unknown call?
    if (isset($this->methods[$actc])) {
      $o = call_user_func($this->methods[$actc], $data, $getData);
      return $o;
    };
  }
