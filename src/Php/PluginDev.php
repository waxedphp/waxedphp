...
class PluginDev extends Plugin {
  ...
  * @return Plugin
  */
  public function setNodeJsPath(string $nodeJsPath): PluginDev {
  ...
  * @return Plugin
  */
  public function uses(array|string $listing): PluginDev {
  ...
  public function inMethod(string $class, string $func):PluginDev {
    $cfg = new Config();
    $d = $cfg->getDataDir();
    if ((is_file($d . '/uses.json')) && (is_writable($d . '/uses.json'))) {
      $a = json_decode(file_get_contents($d . '/uses.json'), true);
      $u = $this->getUsesAsString();
      if (isset($a[$class.'::'.$func])) {
        if ($a[$class.'::'.$func]==$u) return $this;
      }
      $a[$class.'::'.$func]=$u;
      file_put_contents($d . '/uses.json', json_encode($a, JSON_PRETTY_PRINT));
    }
    return $this;
  }

  public function allUses():array {
    $re = [];
    $cfg = new Config();
    $d = $cfg->getDataDir();
    if ((is_file($d . '/uses.json')) && (is_writable($d . '/uses.json'))) {
  ...
  * @return void
  ...
  protected function _prepare_build(): void {
    ...
    if ((!is_dir($path))||(!is_writable($path))) return;
    ...
    chmod($path.'/loader.js', 0666);

  }
  */

  public function prepare_build(): void {
    if (!$this->writablePath) throw new \Exception('Writable path is not set.');
    $plug = implode('-', $this->_uses_);
    $path = $this->writablePath . '/' . $plug;
    //die($path);
  ...
  */
  public function read(string $name): array {
    ...
    if (empty($dep)) throw new \Exception('Wrong plugin: ' . $name);
    foreach ($dep as $key => $val) {
      if (!in_array((string)$key, ['css', 'js', 'PATH'])) {
        if (is_string($val)) {
          $this->find_nonstandard_path($val, $ret);
        } else {
          //print_r($name);print_r($key);die();
        }
      } else if (($key!='PATH')&&(count($dep[$key]))) {
        foreach ($dep[$key] as $dp) {
          $this->find_standard_path($dp, $key, $ret, $dep['PATH']);
        };
      }
    }
    $ret['PATH'] = realpath($dep['PATH']);
    //print_r($ret);
    return $ret;
  }
  ...
  * @return void
  */
  function find_standard_path(string $dp, string $key, array &$ret, $path): void {
    $pu = parse_url($dp);
  ...
  public function load(string $key, array $arr): string {
    ...
    if ((is_array($arr)) && (isset($arr[$key]))) {
      foreach ($arr[$key] as $script) {
        ...
        } elseif (($fname3) && (file_exists($fname3))) {
          $s .= file_get_contents($fname3) . "\n";
        } else {
