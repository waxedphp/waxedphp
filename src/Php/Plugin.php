...
class Plugin {
  ...
  * @return Plugin
  */
  public function setVendorPath(string $vendorPath): Plugin {
    $rpath = realpath($vendorPath);
    if (!$rpath) throw new \Exception('Wrong vendor path: '. $vendorPath);
    $this->vendorPath = $rpath;
    return $this;
  }
  ...
  */
  ...
  public function inMethod(string $class, string $func): Plugin {
    return $this;
  }
  ...
  */
  public function get_example_data(string $name): array {
    $d = new Dependency($this->base, $name);
    return $d->get_example_data();
    /*
    $x = [];
  ...
  */
  public function get_dependency(string $name): array {
    if (!isset($this->dependencies[$name])) {
      $this->dependencies[$name] = new Dependency($this->base, $name);
    }
    return $this->dependencies[$name]->getAllFiles();
    
    
    $f=$this->packagePath.$name;
    if (!is_dir($f)) {
      $composerPackage = 'waxed/'.$name;
    ...
    $ff=$f.DIRECTORY_SEPARATOR . 'dependency.php';
